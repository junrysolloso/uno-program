<?php defined( 'ABSPATH' ) or exit(); ?>
                </div>
              </div>
            </div>
            <!-- end content-wrapper -->

            <!-- start footer -->
            <footer class="footer">
              <div class="container-fluid clearfix">
                <span class="text-muted d-block text-center text-sm-left d-sm-inline-block">&copy; <?php echo date( 'Y' ); ?> UNO</span>
                <span class="float-none text-muted float-sm-right d-block mt-1 mt-sm-0 text-center">Powered by Dinagat Coder</span>
              </div>
            </footer>
            <!-- end footer -->
          </div>
          <!-- end main-panel -->
        </div>
        <!-- end page-body-wrapper -->
    </div>
    <!-- end auto-from-wrapper -->
  </div>
  <!-- end container-scroller -->
  
  <script type="text/javascript" src="<?php uno_e_theme_uri(); ?>assets/js/plugins.bundle.min.js?v=<?php uno_e_version(); ?>"></script>
  <script type="text/javascript" src="<?php uno_e_theme_uri(); ?>assets/js/helpers.bundle.js?v=<?php uno_e_version(); ?>"></script>
  <?php apply_filters( 'uno_custom_js', 'js' ); ?>

</body>
</html>
